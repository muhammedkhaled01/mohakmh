...
class Pdf
{
     ...
     * @return bool
     */
    public function saveImage($pathToImage, $book_id)
    {
        ...
        $parts = explode($delimiter, $pathToImage);
        ...
        // عمل دالة جديدة للتحديث

        /* $bookstatus = Book::where('id', '51')->first();
        $bookstatus->update([
            'status' => 'active',
        ]);  */
     ...
     */
    public function saveAllPagesAsImages($directory, $prefix = '', $book_id)
    {
        $numberOfPages = $this->getNumberOfPages();
        ...
        return array_map(function ($pageNumber = 2) use ($directory, $prefix, $book_id, $numberOfPages) {
            $this->setPage($pageNumber);

            $destination = "{$directory}/{$prefix}{$pageNumber}.{$this->outputFormat}";

            $this->saveImage($destination, $book_id);

            // Check if it's the last page and do something
            // if (1 == 1) {
            //     $bookstatus = Book::where('id', '51')->first();
